...
import { ThumbsUp, ThumbsDown, Clock, CheckCircle, AlertTriangle, Bot, Image as ImageIcon, Upload } from 'lucide-react';
...
const PostCard: React.FC<PostCardProps> = ({ plan, isCurrentUser }) => {
  ...
  const description = parts.length > 1 ? parts.slice(1).join(' :: ') : plan.content;
  ...
  return (
    <div className="bg-brand-card border border-brand-border rounded-2xl p-6 mb-6 hover:border-zinc-600 transition-colors relative overflow-hidden group">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 className="font-bold text-white text-lg leading-none">User {plan.user}</h3>
          <span className="text-xs text-gray-400 uppercase tracking-wider font-semibold">The Challenger</span>
        </div>
      </div>

      {/* Content */}
      <div className="mb-6">
        <h2 className="text-xl md:text-2xl font-black text-white mb-2 italic">{title}</h2>
        ...
        {plan.image && (
            <div className="mt-4 rounded-xl overflow-hidden border border-zinc-800">
                <img src={BACKEND_MEDIA_PATH+plan.image} alt="Plan Goal" className="w-full h-64 object-cover" />
            </div>
        )}
      ...
      {/* Proof Section */}
      {postStatus === 'completed' && plan.proof_media && (
        <div className="mb-6 mt-4 rounded-xl overflow-hidden border border-neon-green/30 relative">
            ...
            <img src={BACKEND_MEDIA_PATH+plan.proof_media} alt="Proof" className="w-full h-64 object-cover" />
        </div>
      )}
      ...
      {isCurrentUser && postStatus === 'wellsee' && (
        <div className="mb-6">
             {!showProofUpload ? (
                 <button onClick={() => setShowProofUpload(true)} className="text-xs bg-zinc-800 hover:bg-zinc-700 px-3 py-2 rounded text-white flex items-center gap-2">
                    <CheckCircle className="w-3 h-3" /> Mark as Complete (Upload Proof)
                 </button>
             ) : (
                 <div className="bg-zinc-900 p-3 rounded-lg border border-zinc-700 flex gap-2 items-center">
                    <input type="file" onChange={e => setProofFile(e.target.files?.[0] || null)} className="text-xs text-gray-400" />
                    <button onClick={handleUploadProof} className="bg-neon-green text-black text-xs px-3 py-1 rounded font-bold">Submit</button>
                    <button onClick={() => setShowProofUpload(false)} className="text-gray-500 text-xs">Cancel</button>
                 </div>
             )}
        </div>
      )}
